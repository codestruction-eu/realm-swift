...
class PBSClientResetTests: ClientResetTests {
    ...
    func testClientResetRecoverOrDiscardLocalFailedRecovery() throws {
        ...
        let app = self.app(id: appId)
        let user = try prepareClientReset(app: app)
...
//
    ...
    func prepareClientReset(app: App? = nil) throws -> User {
        ...
        // Initialize the local file so that we have conflicting history
        try autoreleasepool {
            var configuration = user.flexibleSyncConfiguration() { subscriptions in
            ...
            // Add an object to the local realm that won't be synced due to the suspend
            try realm.write {
                realm.add(SwiftPerson(firstName: "John", lastName: self.name))
            }
            XCTAssertEqual(realm.objects(SwiftPerson.self).count, 1)
