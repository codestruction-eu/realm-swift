...
- (void)testAddObjectsMultipleApps {
    ...
    NSString *appId2;
    if (self.isParent) {
        // Using the same prefix so that they share collections (but still
        // shouldn't see eachother's objects as they're different apps)
        appId1 = [RealmServer.shared createAppWithPartitionKeyType:@"string" prefix:@"testAddObjectsMultipleApps" types:@[Person.self] persistent:false error:nil];
        appId2 = [RealmServer.shared createAppWithPartitionKeyType:@"string" prefix:@"testAddObjectsMultipleApps" types:@[Person.self] persistent:false error:nil];
...
- (void)testIncomingSyncWritesTriggerNotifications {
    ...
    RLMRealm *writeRealm = [self asyncOpenRealmWithConfiguration:[self.createUser configurationWithTestSelector:_cmd]];
...
- (void)testMultipleRealmsSessions {
    ...
    XCTAssertEqual(user.allSessions.count, 3);
