...
def create_workflow_request(type, name, xcode_version, prefix = "")
    action = get_action_for_target(type, name)
...
def create_new_release_workflows
    ...
    if user_input == "y"
        ...
        workflows_to_create.each { |workflow|
            name = workflow['target']
            version = workflow['version']
            puts "Creating new release workflow for target: #{name} and version #{version} for release"
            workflow_id = create_workflow('build', name, version, 'release_')
            puts "https://appstoreconnect.apple.com/teams/#{TEAM_ID}/frameworks/#{product_id}/workflows/#{workflow_id}"
        }
    else
        puts "No"
...
def download_artifact_for_build(build_run)
    actions = get_build_actions(build_run)
...
opts = GetoptLong.new(
    ...
    [ '--find-branch', GetoptLong::REQUIRED_ARGUMENT ],
